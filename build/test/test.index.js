...
import { LinkChecker, LinkState, check, } from '../src/index.js';
...
describe('linkinator', () => {
    ...
    it('should include a redirectUrl when it finds a redirect', async () => {
        ...
        assert.ok(results.passed);
        const redirectedLinks = results.links.filter((x) => x.state === LinkState.REDIRECT);
        assert.strictEqual(redirectedLinks.length, 1);
        assert.strictEqual(redirectedLinks[0].redirectUrl, 'http://redirect2.local/');
        scope.done();
        scope2.done();
    });
