...
describe('config', () => {
    ...
    it('should throw with a reasonable message if the path doesnt exist', async () => {
        ...
        await assert.rejects(getConfig({ config: '/path.dot/does/not/exist' }), /ENOENT: no such file or directory/);
    });
    it('should throw a reasonable message if invalid extension is passed', async () => {
        await assert.rejects(getConfig({ config: 'invalid_extension.cfg' }), /Config file should be either of extensions/);
    });
    ...
    describe('json config file', () => {
        ...
        it('should throw with reasonable message if json file is in invalid format ', async () => {
            ...
            await assert.rejects(getConfig({ config: configPath }), /SyntaxError:.+in JSON/);
        });
    });
